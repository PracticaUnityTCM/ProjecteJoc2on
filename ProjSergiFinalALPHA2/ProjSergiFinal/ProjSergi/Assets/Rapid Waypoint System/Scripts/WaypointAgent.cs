...
public class WaypointAgent : MonoBehaviour {
    ...
    {
       // Debug.Log(transform.parent.GetComponent<WaypointManager>());
        
       // transform.rotation = Quaternion.Euler(new Vector3(-90f, 90f, 0));
    ...
    public virtual void Update()
    {
        ...
        if (EnemyController.EnemyShipBehaibour == EnemyShipBehaivour.Patroling)
        {
            transform.parent = transform;
            WaypointMovementUpdate();
        }
        else if (EnemyController.EnemyShipBehaibour == EnemyShipBehaivour.Following)
        {
            if (!hasChange)
            {
                hasChange = true;
